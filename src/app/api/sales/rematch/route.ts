...
export async function POST(){
  const {sales,out}=await candidates(); let matched=0,skipped=0;
  for(const x of out){const qty=x.line.quantity||1;if(x.match.item.quantity<qty){skipped++;continue;}
    await prisma.$transaction(async tx=>{const fresh=await tx.sale.findUnique({where:{id:x.sale.id},include:{lines:true}});if(!fresh||fresh.status!==SaleStatus.NEW)return;const fl=fresh.lines[0];if(!fl||fl.inventoryItemId)return;await tx.saleLine.update({where:{id:fl.id},data:{inventoryItemId:x.match.item.id}});await tx.sale.update({where:{id:x.sale.id},data:{status:SaleStatus.MATCHED,matchMethod:x.match.method,matchConfidence:x.match.confidence}});matched++;});
  }
  return NextResponse.json({ok:true,checked:sales.length,matched,skipped,note:"Historical baseline matched without decrementing current active inventory."});
}
