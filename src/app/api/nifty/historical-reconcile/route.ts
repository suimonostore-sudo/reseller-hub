...
export async function GET(req:NextRequest){let client:any=null;try{
 ...
 const monthRaw=req.nextUrl.searchParams.get("month");const month=monthRaw?Number(monthRaw):null;if(month!=null&&(!Number.isInteger(month)||month<1||month>12))return NextResponse.json({ok:false,error:"month must be 1-12"},{status:400});
 const orderMap=new Map<string,any>(),windowStats:any[]=[];
 for(const w of monthWindows(month??undefined)){let raw=0,pages=0;for(let page=0;page<30;page++){const r=await client.callTool({name:"search_orders",arguments:{startDate:w.start.toISOString().replace("Z",""),endDate:w.end.toISOString().replace("Z",""),sort:"sold_at",sortOrder:"desc",page,limit:50}});if(r?.isError)throw new Error(`Nifty search_orders ${w.label} error: ${JSON.stringify(r?.content||r).slice(0,1000)}`);const payload=jsonFromResult(r),batch=ordersFrom(payload);pages++;raw+=batch.length;for(const o of batch)orderMap.set(niftyOrderIdentity(o),o);if(payload?.hasMore===false||(payload?.hasMore==null&&batch.length<50))break}windowStats.push({month:w.label,pages,rawOrders:raw})}
