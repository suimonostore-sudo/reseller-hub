...
export async function GET(req:NextRequest){let client:any=null;try{
 ...
 const proposals:any[]=[],diagnostics:any[]=[];let rejectedTitle=0,rejectedSize=0,exactOrderIdHits=0,rejectedAmount=0,noSku=0,noInventoryForSku=0,duplicateInventoryForSku=0,uniqueInventoryForSku=0;
 for(const o of orders){const p=platform(pick(o,"marketplace","platform"));if(!p)continue;const soldAt=new Date(pick(o,"soldAt","sold_at")||0);if(!Number.isFinite(soldAt.getTime()))continue;const raw=linesFrom(o);if(raw.length!==1)continue;const line=raw[0],niftyTitle=String(pick(line,"title","itemTitle","item_title","name")||"");const external=String(pick(o,"externalMarketplaceOrderId","external_marketplace_order_id","externalOrderId","external_order_id","orderId","order_id")||"").trim();const exact=(byOrder.get(orderKey(external))||[]).filter((s:any)=>String(s.platform)===p);if(exact.length!==1)continue;exactOrderIdHits++;const sale=exact[0];
  ...
  const sku=String(pick(line,"sku","SKU")||pick(o,"sku","SKU")||"").trim();if(!sku){noSku++;diagnostics.push({saleId:sale.id,external,reason:"NO_NIFTY_SKU",saleTitle:sale.lines?.[0]?.title,niftyTitle});continue}
  const candidates=bySource.get(norm(sku))||[];if(!candidates.length){noInventoryForSku++;diagnostics.push({saleId:sale.id,external,reason:"NO_INVENTORY_SOURCE_SKU",sourceSku:sku,saleTitle:sale.lines?.[0]?.title,niftyTitle});continue}
  if(candidates.length>1){duplicateInventoryForSku++;const viable=candidates.filter((i:any)=>!sizeConflict(i.title,niftyTitle)&&titleAgrees(i.title,niftyTitle));if(viable.length!==1){diagnostics.push({saleId:sale.id,external,reason:viable.length?"MULTIPLE_VIABLE_INVENTORY":"NO_VIABLE_INVENTORY",sourceSku:sku,saleTitle:sale.lines?.[0]?.title,niftyTitle,candidates:candidates.slice(0,8).map((i:any)=>({id:i.id,sku:i.sku,title:i.title,quantity:i.quantity,status:i.dispositionStatus,score:titleScore(i.title,niftyTitle),sizeConflict:sizeConflict(i.title,niftyTitle)}))});continue}candidates.splice(0,candidates.length,viable[0])}else uniqueInventoryForSku++;
  const inv=candidates[0];if(sizeConflict(inv.title,niftyTitle)){rejectedSize++;continue}if(!titleAgrees(inv.title,niftyTitle)){rejectedTitle++;continue}
