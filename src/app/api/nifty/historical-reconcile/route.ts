...
export async function GET(req:NextRequest){
  ...
  try{
    ...
    const proposals:any[]=[];let rejectedTitle=0;
    for(const o of orders){
      ...
      const near=newSales.filter((s:any)=>String(s.platform)===p&&Math.abs(new Date(s.soldAt).getTime()-soldAt.getTime())<=15*60*1000&&titleAgrees(s.lines?.[0]?.title,niftyTitle));
      if(near.length!==1){if(newSales.some((s:any)=>String(s.platform)===p&&Math.abs(new Date(s.soldAt).getTime()-soldAt.getTime())<=15*60*1000))rejectedTitle++;continue}
      ...
      const inv=candidates[0];if(!titleAgrees(inv.title,niftyTitle)){rejectedTitle++;continue}
    ...
    return NextResponse.json({ok:true,readOnly:true,niftyOrders:orders.length,newSales:newSales.length,safeUniqueProposals:unique.size,conflictsRemoved:conflicts.size,rejectedTitle,proposals:[...unique.values()]});
