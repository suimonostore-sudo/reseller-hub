...
const categoryRules:[string,RegExp][]=[
 ...
 ["SHORTS",/\bshorts\b/i],
 ["SHIRT",/\b(?:shirt|shirts|polo|polos|tee|t-shirt|tshirt|henley)\b/i],
...
function categories(s:any){const x=String(s||"");return new Set(categoryRules.filter(([,r])=>r.test(x)).map(([k])=>k))}
...
function strongTitleCandidates(inventory:any[],title:string){return inventory.map(i=>({i,score:titleScore(i.title,title)})).filter(x=>x.score>=0.88&&!sizeConflict(x.i.title,title)&&!genderConflict(x.i.title,title)&&!categoryConflict(x.i.title,title)).sort((a,b)=>b.score-a.score||a.i.id-b.i.id)}
...
export async function GET(req:NextRequest){let client:any=null;try{
 ...
 for(const o of orders){const p=platform(pick(o,"marketplace","platform"));if(!p)continue;const soldAt=new Date(pick(o,"soldAt","sold_at")||0);if(!Number.isFinite(soldAt.getTime()))continue;const raw=linesFrom(o);if(raw.length!==1)continue;const line=raw[0],niftyTitle=String(pick(line,"title","itemTitle","item_title","name")||"");const external=String(pick(o,"externalMarketplaceOrderId","external_marketplace_order_id","externalOrderId","external_order_id","orderId","order_id")||"").trim();const exact=(byOrder.get(orderKey(external))||[]).filter((s:any)=>String(s.platform)===p);if(exact.length!==1)continue;exactOrderIdHits++;const sale=exact[0];
  ...
  if(!sku)noSku++;
  if(!candidates.length){if(sku)noInventoryForSku++;const fallback=strongTitleCandidates(inventory,niftyTitle);const best=fallback[0],second=fallback[1];const uniqueStrong=!!best&&best.score>=0.92&&(!second||best.score-second.score>=0.12);if(uniqueStrong){candidates=[best.i];basis="EXACT_ORDER_ID_STRONG_TITLE_SIZE_GENDER_CATEGORY";titleFallbackUnique++}else{if(fallback.length)titleFallbackAmbiguous++;else titleFallbackNone++;diagnostics.push({saleId:sale.id,external,reason:fallback.length?"TITLE_FALLBACK_AMBIGUOUS":"TITLE_FALLBACK_NONE",sourceSku:sku||null,saleTitle:sale.lines?.[0]?.title,niftyTitle,fallback:fallback.slice(0,6).map(x=>({id:x.i.id,sku:x.i.sku,sourceSku:x.i.sourceSku,title:x.i.title,quantity:x.i.quantity,status:x.i.dispositionStatus,score:x.score,categories:[...categories(x.i.title)]}))});continue}}
  if(candidates.length>1){duplicateInventoryForSku++;const viable=candidates.filter((i:any)=>!sizeConflict(i.title,niftyTitle)&&!genderConflict(i.title,niftyTitle)&&!categoryConflict(i.title,niftyTitle)&&titleAgrees(i.title,niftyTitle));if(viable.length!==1){diagnostics.push({saleId:sale.id,external,reason:viable.length?"MULTIPLE_VIABLE_INVENTORY":"NO_VIABLE_INVENTORY",sourceSku:sku,saleTitle:sale.lines?.[0]?.title,niftyTitle,candidates:candidates.slice(0,8).map((i:any)=>({id:i.id,sku:i.sku,title:i.title,quantity:i.quantity,status:i.dispositionStatus,score:titleScore(i.title,niftyTitle),sizeConflict:sizeConflict(i.title,niftyTitle),genderConflict:genderConflict(i.title,niftyTitle),categoryConflict:categoryConflict(i.title,niftyTitle)}))});continue}candidates=[viable[0]]}else if(basis==="EXACT_ORDER_ID_SKU_TITLE")uniqueInventoryForSku++;
