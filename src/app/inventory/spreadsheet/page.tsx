...
export default function SpreadsheetInventoryPage(){
 ...
 const filtered=useMemo(()=>{const q=search.trim().toLowerCase();return items.filter(i=>{const st=workflow(i);const matchStatus=status==="ALL"||st===status;const text=[i.sku,i.sourceSku,i.title,i.purchaseStore,i.condition,i.location,i.dispositionNote,st].filter(Boolean).join(" ").toLowerCase();return matchStatus&&(!q||text.includes(q))})},[items,search,status]);
 return <main className="shell">
  ...
  <header className="header"><div><a className="back" href="/inventory">← Inventory</a><h1>Inventory Spreadsheet</h1><p>All inventory in one Excel-style table.</p></div><div className="actions"><a className="button secondary" href="/inventory/bulk-entry">Quick Add</a><a className="button" href="/inventory">Inventory Actions</a></div></header>
