...
const statusOrder=["MATCHED","PICKED","PACKED","SHIPPED"];
...
export default function PickPackPage(){
  ...
  const displayed=filter==="READY"?ready:sales.filter(s=>s.status===filter);
  ...
  function printPickList(){
    ...
    const labels=rows.flatMap(s=>s.lines.map(l=>{
      ...
      const sub=matched?(l.inventoryItem!.location||""):l.title;
      return `<div class="label"><div class="main">${esc(main)}</div><div class="sub">${esc(sub)}</div></div>`;
    })).join("");
    const w=window.open("","_blank");
    if(!w){setMessage("Allow pop-ups to print.");return}
    w.document.write(`<html><head><title>Pick List</title><style>@page{size:1in 4in;margin:0}body{margin:0;font-family:Arial}.label{width:4in;height:1in;box-sizing:border-box;border-bottom:1px dashed #999;padding:.1in .12in;display:flex;flex-direction:column;justify-content:center;overflow:hidden}.main{font-size:25px;font-weight:800;line-height:1}.sub{font-size:12px;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}@media print{.label{break-after:page}}</style></head><body>${labels}</body></html>`);
  ...
  return <main className="shell">
    <header className="header"><div><a className="back" href="/sales">← Sales</a><h1>Pick & Pack</h1><p>Pick matched orders by SKU, then pack and move them through fulfillment.</p></div>
      <div className="actions"><button className="secondary" onClick={printPickList}>Print 1×4 Pick Labels</button><button className="secondary" onClick={printShippingFallback}>Print Shipping Fallback</button></div>
    </header>
    ...
    <section className="cards">
      ...
      <div className="card"><span>Selected</span><strong>{selected.length}</strong></div>
    </section>

    <section className="panel">
      <div className="pickToolbar">
        <div className="tabs">{["READY","MATCHED","PICKED","PACKED","SHIPPED"].map(x=><button key={x} className={filter===x?"tab active":"tab"} onClick={()=>setFilter(x)}>{x}</button>)}</div>
        <div className="actions"><button className="secondary" onClick={selectAll}>Select All</button><button className="secondary" onClick={clear}>Clear</button>
          {selected.length>0&&<><button onClick={()=>batch("PICKED")} disabled={busy}>Mark Picked</button><button onClick={()=>batch("PACKED")} disabled={busy}>Mark Packed</button></>}</div>
      </div>

      <table><thead><tr><th></th><th>Status</th><th>SKU</th><th>Location</th><th>Item</th><th>Buyer</th><th>Platform</th><th>Order</th><th></th></tr></thead>
      <tbody>{displayed.map(s=>s.lines.map((l,idx)=><tr key={`${s.id}-${idx}`}>
        ...
        <td>{l.inventoryItem?.location||"—"}</td>
    ...
    <section className="panel">
      <div className="panelHead"><h2>Workflow</h2><span>Designed for SKU-first picking</span></div>
      <div className="flow"><div><b>1</b><br/>Match sale</div><b>→</b><div><b>2</b><br/>Print 1×4 SKU label</div><b>→</b><div><b>3</b><br/>Pick & label poly/box</div><b>→</b><div><b>4</b><br/>Print shipping labels</div><b>→</b><div><b>5</b><br/>Mark shipped</div></div>
    </section>
  </main>
}
