...
function chooseListedPrefix<T extends {title:string;workflowStatus?:string|null}>(title:string,candidates:T[]){
  ...
  const listed=hits.filter(item=>item.workflowStatus==="LISTED");
...
export async function findBestMatch(platform:Platform, externalListingId:string|undefined, title:string, sku?:string){
  ...
  const listedPrefixListing=chooseListedPrefix(title,listingCandidates);
  if(listedPrefixListing) return {item:listedPrefixListing.item.item,method:"LISTED_PREFIX_LISTING",confidence:listedPrefixListing.score};
  ...
  const listedPrefix=chooseListedPrefix(title,items);
  if(listedPrefix) return {item:listedPrefix.item,method:"LISTED_PREFIX_INVENTORY",confidence:listedPrefix.score};
